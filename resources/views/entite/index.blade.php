...
<div class="flex items-center">
    <div class="rounded-full p-1.5 text-white ring-1 ltr:mr-3 rtl:ml-3" style="background-color: #67152e;">
        <!-- Nouvelle icône représentant une entité -->
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M3 21V7a1 1 0 011-1h16a1 1 0 011 1v14" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M9 21V10H15V21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M3 21H21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M7 14H9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M15 14H17" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
        </svg>
    </div>
    <!-- Texte -->
    <span class="font-semibold text-lg" style="color: #67152e;">LISTE DES ENTITES</span>
    </div>
            ...
            <a href="/entites" >Ajouter entité</a>
        </button>
        @endcan
        </div>
        <div class="panel">
            <h5 class="md:absolute md:top-[25px] md:mb-0 mb-5 font-semibold text-lg dark:text-white-light">Entités</h5>
            <div class="relative">
...
<script>
    document.addEventListener("alpine:init", () => {
        Alpine.data("miscellaneous", () => ({
            ...
            showCols: [0, 1, 2, 3],
            datatable1: null,
            entites: @json($entites), // Injecter les entites depuis Laravel

            init() {
                // Vérifiez que les entites sont bien injectées
                ...
                // Générer les données pour chaque entite
                let data = this.entites.map((entite) => [
                    entite.id,
                    entite.nom,
                    entite.code,

                    formatDate(entite.created_at), // Formater la date
                    `
                        <ul class="flex items-center justify-center gap-2">
                            <!-- Icône pour éditer -->
                            <li>
                                <a href="/entites/${entite.id}/edit" x-tooltip="Edit">
                            ...
                            <li>
                                <form action="/entites/${entite.id}" method="POST" style="display:inline;" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette entite ?');">
