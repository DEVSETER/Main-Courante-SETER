            ...
            <div
                ...
                <div class="relative flex flex-col justify-center rounded-md bg-white/60 backdrop-blur-lg dark:bg-black/50 px-6 lg:min-h-[758px] py-20">
                    ...
                    <div x-data="form" class="mx-auto w-full max-w-[440px]">
                        <div class="mb-10">
                            <h1 class="text-3xl font-extrabold uppercase !leading-snug text-primary md:text-4xl"> nouvel utilisateur</h1>
                            <p class="text-base font-bold leading-normal text-white-dark">Enter your email and password to register</p>
                        </div>
                        <form  class="space-y-5 dark:text-white "  id="formUtilisateur" @submit.prevent="submitForm3()" action="{{ route('users.store') }}"  method="POST">
                            @csrf

                            <div :class="[isSubmitForm3 ? (form3.nom ? 'has-success' : 'has-error') : '']">
                                <label for="nom">Nom</label>
                                <div class="relative text-white-dark">
                                    <input id="nom" name="nom" type="text" x-model="form3.nom" placeholder="entrez le nom" class="form-input ps-10 placeholder:text-white-dark" />
                                    {{-- @error('nom')
                            ...
                            <div :class="[isSubmitForm3 ? (form3.prenom ? 'has-success' : 'has-error') : '']">
                                <label for="prenom">Prenom</label>
                                <div class="relative text-white-dark">
                                    <input id="prenom" type="text" name="prenom" x-model="form3.prenom" placeholder="entrez le prenom" class="form-input ps-10 placeholder:text-white-dark" />
                            ...
                            <div  :class="[isSubmitForm3 ? (form3.select ? 'has-success' : 'has-error') : '']">
                                ...
                                <div class="relative text-white-dark">
                                    <input id="telephone" name="telephone" type="tel" placeholder="77-123-45-67" x-model="form3.telephone" class="form-input ps-10 placeholder:text-white-dark" />
                                    @error('telephone')
                                    <div class="text-danger">{{ $message }}</div>
                                              @enderror
                                    <span class="absolute start-4 top-1/2 -translate-y-1/2">
                                ...
                                {{-- <form class="space-y-5" @submit.prevent="submitForm3()"> --}}
                                    <div :class="[isSubmitForm3 ? (form3.select ? 'has-success' : 'has-error') : '']">
                                        ...
                                        <select class="form-select text-white-dark " name="fonction" x-model="form3.fonction">
                                            <option value="">Sélectionner une fonction</option>
                                            <option value="CIV">CIV</option>
                                            <option value="HOTLINE">HOTLINE</option>
                                            <option value="SUPERVISEUR COF">SUPERVISEUR COF</option>
                                            <option value="CM">CM</option>
                                            <option value="PLANIFICATEUR">PLANIFICATEUR</option>
                                            <option value="CHEF CIRCULATION">CHEF CIRCULATION</option>
                                        </select>

                                        <template x-if="isSubmitForm3 && form3.fonction">
                            ...
                            {{-- <div x-data="form"> --}}
                                {{-- <form class="space-y-5" @submit.prevent="submitForm3()"> --}}
                                    <div :class="[isSubmitForm3 ? (form3.select ? 'has-success' : 'has-error') : '']">
                                        <select class="form-select text-white-dark" name="direction" x-model="form3.direction">
                                            <option>Sélectionner une direction</option>
                                            <option>DSI</option>
                                            <option>DEX</option>
                                            <option>DMSV</option>
                                            <option>DMI</option>
                                        ...
                                        <template x-if="isSubmitForm3 && form3.direction">
                                            <p class="text-[#1abc9c] mt-1">Example valid custom select feedback</p>
                                        </template>
                                        <template x-if="isSubmitForm3 && !form3.direction">
                                            <p class="text-danger mt-1">Please Select the field</p>
                                ...
                                {{-- </form> --}}
                                    ...
                                    {{-- <div :class="[isSubmitForm3 ? (form3.select ? 'has-success' : 'has-error') : '']">
                                        <select class="form-select text-white-dark" name="role_id" x-model="form3.role_id">
                                            <option >Sélectionner un profil</option>
                                            <option value="5">UTILISATEUR SIMPLE</option>
                                            <option value="4"> ADMIN</option>
                                            <option value="3">SUPER ADMIN</option>

                                        </select>

                                        <template x-if="isSubmitForm3 && form3.role_id">
                                            <p class="text-[#1abc9c] mt-1">Example valid custom select feedback</p>
                                        </template>
                                        <template x-if="isSubmitForm3 && !form3.role_id">
                                            <p class="text-danger mt-1">Please Select the field</p>
