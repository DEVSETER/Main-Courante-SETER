...
<meta name="csrf-token" content="{{ csrf_token() }}">
 ...
 <div x-data="miscellaneous">
    ...
    <div class="rounded-full p-1.5 text-white ring-1 ltr:mr-3 rtl:ml-3" style="background-color: #67152e;">
        <!-- Icône représentant les permissions -->
    ...
    <!-- Texte -->
    <span class="font-semibold text-lg" style="color: #67152e;">LISTE LIEUX</span>
</div>
            ...
            <button  class="btn btn-primary flex items-center" style="background-color: #67152e; border-color: #67152e; color: #fff;">
                ...
                <a href="/locations/create">Ajouter un lieu</a>
            </button>
            @endcan
        </div>
        <div class="panel">
            <h5 class="md:absolute md:top-[25px] md:mb-0 mb-5 font-semibold text-lg dark:text-white-light">Lieux</h5>
            <div class="relative">
...
<script>
    document.addEventListener("alpine:init", () => {
        Alpine.data("miscellaneous", () => ({
            ...
            showCols: [0, 1, 2, 3],
            datatable1: null,
            permissions: @json($locations), // Injecter les permissions depuis Laravel

            init() {
                // Vérifiez que les permissions sont bien injectées
                ...
                // Générer les données pour chaque lieu
                let data = this.permissions.map((locations) => [
                        locations.id,
                    locations.libelle,


                    formatDate(locations.created_at), // Formater la date
                    `
                        <ul class="flex items-center justify-center gap-2">
                            <!-- Icône pour éditer -->
                            <li>
                                <a href="/locations/${locations.id}/edit" x-tooltip="Edit">
                            ...
                            <li>
                                <form action="/locations/${locations.id}" method="POST" style="display:inline;" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce lieu ?');">
