            ...
            <div
                ...
                <div class="relative flex flex-col justify-center rounded-md bg-white/60 backdrop-blur-lg dark:bg-black/50 px-6 lg:min-h-[758px] py-20">
                    ...
                    <div x-data="form" class="mx-auto w-full max-w-[440px]">
                        <div class="mb-10">
                            <h1 class="text-3xl font-extrabold uppercase !leading-snug text-primary md:text-4xl" style="color: #67152e; border-color: #ebba7d;"> Nouveau lieu </h1>
                            <p class="text-base font-bold leading-normal text-white-dark">Renseigner le nom du lieu puis enrigistrer</p>
                        </div>
                        <form  class="space-y-5 dark:text-white "  id="formUtilisateur" @submit.prevent="submitForm3()" action="{{ route('locations.store') }}"  method="POST">
                            @csrf

                            <div :class="[isSubmitForm3 ? (form3.nom ? 'has-success' : 'has-error') : '']">
                                <label for="libelle">Description du lieu</label>
                                <div class="relative text-white-dark">
                                    <input id="libelle" name="libelle" type="text" x-model="form3.libelle" placeholder="entrez la description du lieu" value="{{ old('libelle') }}" class="form-input ps-10 placeholder:text-white-dark" />
                                    {{-- @error('nom')
