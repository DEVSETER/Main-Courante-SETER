            ...
            <div
                ...
                <div class="relative flex flex-col justify-center rounded-md bg-white/60 backdrop-blur-lg dark:bg-black/50 px-6 lg:min-h-[758px] py-20">
                    ...
                    <div x-data="form" class="mx-auto w-full max-w-[440px]">
                        <div class="mb-10">
                            <h1 class="text-3xl font-extrabold uppercase !leading-snug text-primary md:text-4xl" style="color: #67152e; border-color: #ebba7d;">Mise à jour Privilège</h1>
                            <p class="text-base font-bold leading-normal text-white-dark">Renseignez le nom du privilège puis enregistrez.</p>
                        </div>
                        <form class="space-y-5 dark:text-white" id="formUtilisateur" @submit.prevent="submitForm3()" action="{{ route('permissions.update', $permission->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div :class="[isSubmitForm3 ? (form3.name ? 'has-success' : 'has-error') : '']">
                                ...
                                <div class="relative text-white-dark">
                                    <input type="text" id="name" name="name" value="{{ old('name', $permission->name) }}"  class="form-input ps-10 placeholder:text-white-dark" />
...
<div>
    ...
    <select id="type" name="type" class="form-select mt-1 block w-full">
        ...
        @foreach($types as $type)
            <option value="{{ $type }}" {{ (old('type', $permission->type) == $type) ? 'selected' : '' }}>
                {{ ucfirst($type) }}
            </option>
        @endforeach
    </select>
    ...
    Alpine.data("form", () => ({
        form3: {
            name: "{{ $permission->name }}" // Initialiser avec la valeur actuelle
        },
        isSubmitForm3: false,
        submitForm3() {
            ...
            if (name) {
                this.showMessage('Le privilège a été modifié avec succès.');
