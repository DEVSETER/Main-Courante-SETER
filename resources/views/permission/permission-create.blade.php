...
@endif --}}
    ...
    <div x-data="auth">
        ...
        <div class="relative flex min-h-screen items-center justify-center bg-[url(/assets/images/auth/map.png)] bg-cover bg-center bg-no-repeat px-6 py-10  sm:px-16">
            ...
            <div
                class="relative w-full max-w-[870px] rounded-md bg-[linear-gradient(45deg,#fff9f9_0%,rgba(255,255,255,0)_25%,rgba(255,255,255,0)_75%,_#fff9f9_100%)] p-2">
                        ...
                        <form  class="space-y-5 dark:text-white "  id="formUtilisateur" @submit.prevent="submitForm3()" action="{{ route('permissions.store') }}"  method="POST">
                            ...
                            <div :class="[isSubmitForm3 ? (form3.nom ? 'has-success' : 'has-error') : '']">
                                ...
                                <div class="relative text-white-dark">
                                    <input id="name" name="name" type="text" x-model="form3.name" placeholder="entrez le nom du privilège" value="{{ old('name') }}" class="form-input ps-10 placeholder:text-white" />
                                    {{-- @error('nom')
