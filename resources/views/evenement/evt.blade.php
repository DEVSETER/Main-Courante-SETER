...
<div class="pagination-container" x-show="totalItems > 0">
    ...
    <div x-show="totalPages > 15" class="text-center text-sm text-gray-500" style="margin-top: 10px;">
        ...
        <div class="modal" x-show="showModal" @click.away="closeModal()">
        </div>
...
<template x-for="field in getCurrentFields()" :key="field.key">
    <div>
        <!-- Séparateur normal -->
        <div x-show="field.type === 'separator' && field.key !== 'separator-actions' && field.key !== 'separator-comments'" class="form-separator">
            <h4 x-text="field.label"></h4>
        </div>

        <!-- Séparateur pour actions existantes - SEULEMENT en édition -->
        <div x-show="field.key === 'separator-actions' && isEditing" class="form-separator">
            <h4 x-text="field.label"></h4>
        </div>

        <!-- Séparateur pour commentaires existants - SEULEMENT en édition -->
        <div x-show="field.key === 'separator-comments' && isEditing" class="form-separator">
            <h4 x-text="field.label"></h4>
        </div>
        ...
        <!-- Liste des actions existantes - SEULEMENT en édition -->
              <!-- Liste des actions existantes - SEULEMENT en édition -->
        <div x-show="field.key === 'actions-list' && isEditing" class="existing-items actions-list-container">
            <!-- Debug temporaire -->
            <div style="background: lightblue; padding: 5px; margin: 5px 0; font-size: 11px;">
                DEBUG ACTIONS: <span x-text="(currentEvent?.originalData?.actions || []).length"></span> actions trouvées
            </div>

            <template x-for="(action, actionIndex) in (currentEvent?.originalData?.actions || [])" :key="action.id || actionIndex">
                <div class="bg-white border p-3 mb-2 rounded shadow" :class="{ 'border-blue-500': action.editing }">
                    <!-- En-tête de l'action avec boutons -->
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <!-- Boutons d'action -->
                        <div class="flex items-center gap-2">
                            <span class="text-xs text-gray-500" x-text="action.created_at ? new Date(action.created_at).toLocaleString() : 'Date inconnue'"></span>

                            <!-- Bouton modifier -->
                            <button type="button"
                                    class="text-blue-500 hover:text-blue-700 hover:bg-blue-50 p-1 rounded"
                                    @click="toggleActionEdit(actionIndex)"
                                    :title="action.editing ? 'Annuler modification' : 'Modifier cette action'">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" x-show="!action.editing">
                            ...
                            <button type="button"
                                    ...
                                    class="text-green-500 hover:text-green-700 hover:bg-green-50 p-1 rounded"
                                    @click="saveActionEdit(actionIndex)"
                                    title="Sauvegarder les modifications">
                    ...
                    <!-- Contenu de l'action (mode lecture) -->
                    <div x-show="!action.editing" class="mb-2">
                        <p class="text-sm font-medium text-gray-700" x-text="action.commentaire || 'Aucun commentaire'"></p>
                        <template x-if="action.message && action.message.trim() !== ''">
                            <p class="text-sm text-gray-600 mt-1 italic" x-text="'Message: ' + action.message"></p>
                        </template>
                    </div>

                    <!-- Contenu de l'action (mode édition) -->
                    <div x-show="action.editing" class="mb-2">
                        <div class="form-group mb-2">
                            <label class="form-label text-xs">Type d'action</label>
                            <select class="form-control text-sm" x-model="action.type">
                        ...
                        <div class="form-group mb-2">
                            ...
                            <textarea class="form-control text-sm"
                                      rows="2"
                                      x-model="action.commentaire"
                                      placeholder="Commentaire de l'action..."></textarea>
                        </div>

                        <div class="form-group" x-show="action.type !== 'texte_libre'">
                            <label class="form-label text-xs">Message personnalisé</label>
                            <textarea class="form-control text-sm"
                                      rows="2"
                                      x-model="action.message"
                    ...
                    <div class="flex items-center justify-between text-xs text-gray-500">
                        ...
                        <span x-text="action.auteur ?
                                    ('Par: ' + action.auteur.nom_complet) :
                                    action.auteur_id == window.user.id ?
                                    ('Par: ' + window.user.nom_complet) :
                                    'Auteur inconnu'"></span>
                    </div>
                </div>
            </template>
        ...
        <!-- NOUVEAU : Liste des commentaires existants - SEULEMENT en édition -->
        <div x-show="field.key === 'commentaires-list' && isEditing" class="existing-items comments-list-container">
            <!-- Debug temporaire -->
            <div style="background: yellow; padding: 5px; margin: 5px 0; font-size: 11px;">
                DEBUG: <span x-text="(currentEvent?.originalData?.commentaires || []).length"></span> commentaires trouvés
            ...
            <template x-for="(commentaire, commentIndex) in (currentEvent?.originalData?.commentaires || [])" :key="commentaire.id || commentIndex">
                ...
                <div class="flex items-center justify-between text-xs text-gray-500">
                    <span>Commentaire #<span x-text="commentaire.id || commentIndex + 1"></span></span>
                    <span x-text="commentaire.auteur ?
                                ('Par: ' + commentaire.auteur.nom + ' ' + commentaire.auteur.prenom) :
                                commentaire.redacteur == window.user.id ?
                                ('Par: ' + window.user.prenom + ' ' + window.user.nom) :
                                'Auteur inconnu'"></span>
                </div>
                                </div>
            </template>
        ...
        <!-- Nouveau commentaire - SEULEMENT en édition -->
        <div x-show="field.key === 'new_comment' && isEditing" class="form-group">
            <label class="form-label" x-text="field.label"></label>
            <textarea class="form-control"
                      rows="3"
                      x-model="currentEvent.new_comment"
                      placeholder="Ajouter un commentaire à cet événement..."></textarea>
        </div>

        <!-- Type d'action -->
        <div x-show="field.key === 'type_action'" class="form-group">
            <label class="form-label" x-text="field.label"></label>
            <select class="form-control" x-model="currentEvent.type_action">
        ...
        <!-- Destinataires -->
        <div x-show="field.key === 'destinataires' && (!field.condition || field.condition(currentEvent))" class="form-group">
            <label class="form-label" x-text="field.label"></label>
            <select class="form-control" multiple x-model="currentEvent.destinataires">
                <template x-for="option in field.options">
        ...
        <div x-show="field.key === 'message_personnalise' && (!field.condition || field.condition(currentEvent))" class="form-group">
            ...
            <textarea class="form-control"
                      ...
                      placeholder="Message personnalisé pour cette action..."></textarea>
        </div>
    </div>
</template> <!-- FIN de la boucle des champs -->
    ...
    <div class="form-group" style="display: flex; justify-content: space-between; align-items: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
        <button type="button"
                class="bg-yellow-100 text-yellow-800 hover:bg-yellow-200 hover:text-yellow-900 px-4 py-2 rounded-full text-sm soft-yellow-btn"
                @click="closeModal()">
            ❌ Annuler
        </button>
        <button type="submit"
